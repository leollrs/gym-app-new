    ...
    return (
      <div className="min-h-screen bg-[var(--color-bg-primary)] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-[13px] text-[var(--color-text-muted)]">{t('firstWorkoutWelcome.preparingYourPlan')}</p>
        </div>
      </div>
    );
  }
        ...
        >
          ...
          <motion.div className="mb-4" variants={itemVariants}>
            <button
              ...
              className="w-full flex items-center justify-center gap-3 bg-[#D4AF37] hover:bg-[#E6C766] text-black font-bold text-[14px] py-4 px-6 rounded-[14px] transition-all shadow-[0_0_30px_rgba(212,175,55,0.15)] whitespace-nowrap focus:ring-2 focus:ring-[#D4AF37] focus:outline-none"
            >
            ...
            {!hasRoutines && (
              <p className="text-[12px] text-[var(--color-text-subtle)] text-center mt-2">
          ...
          <motion.div className="mb-8" variants={itemVariants}>
            <button
              ...
              className="w-full text-center text-[14px] text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] py-2 transition-colors focus:ring-2 focus:ring-[#D4AF37] focus:outline-none rounded-lg"
            >
            ...
            <div className="bg-[var(--color-bg-card)] rounded-[14px] border border-white/8 p-5 overflow-hidden">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[14px] font-bold text-[var(--color-text-primary)] mb-1">{t('firstWorkoutWelcome.firstWeekChallenge')}</p>
                ...
                {[1, 2, 3].map((n) => (
                  <div key={n} className="flex items-center gap-2 flex-1">
                    <div className="w-7 h-7 rounded-full border-2 border-white/10 flex items-center justify-center">
                      <span className="text-[11px] font-bold text-[var(--color-text-muted)]">{n}</span>
                    </div>
            ...
            <div className="flex flex-col gap-2.5">
              {QUICK_TIP_KEYS.map((tip) => {
                ...
                return (
                  <motion.div
                    key={tip.titleKey}
                    className="bg-[var(--color-bg-card)] rounded-[14px] border border-white/8 px-4 py-3.5 flex items-start gap-3.5 overflow-hidden"
                    variants={itemVariants}
                  >
                    <div className="flex-shrink-0 w-9 h-9 rounded-lg bg-white/5 flex items-center justify-center mt-0.5">
                      <Icon size={17} className="text-[var(--color-text-muted)]" strokeWidth={2} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[13px] font-semibold text-[var(--color-text-primary)] mb-0.5">{t(tip.titleKey)}</p>
                      <p className="text-[12px] text-[var(--color-text-subtle)] leading-relaxed">{t(tip.descKey)}</p>
                    </div>
