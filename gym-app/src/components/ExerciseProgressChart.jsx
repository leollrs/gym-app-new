  ...
  if (proj?.value != null) {
    return (
      <div className="bg-[var(--color-bg-card)] border border-white/10 rounded-xl px-3 py-2 shadow-xl shadow-black/40 text-[12px]">
        ...
        <p className="font-semibold text-[#D4AF37]">{t('strength.projectedLbs', { value: proj.value, defaultValue: `Projected: ${proj.value} lbs` })}</p>
      </div>
    );
  }
  if (real?.value != null) {
    return (
      <div className="bg-[var(--color-bg-card)] border border-white/10 rounded-xl px-3 py-2 shadow-xl shadow-black/40 text-[12px]">
        <p className="text-[var(--color-text-muted)] text-[11px] mb-1">{label}</p>
        <p className="font-semibold text-[#D4AF37]">{t('strength.estOneRMLbs', { value: real.value, defaultValue: `Est. 1RM: ${real.value} lbs` })}</p>
      </div>
    );
  }
  return null;
...
function ExerciseProgressChart({ exerciseId, exerciseName, onClose }) {
  ...
  return (
    <div
      className="fixed inset-0 z-[160] flex items-center justify-center px-4 bg-black/50 backdrop-blur-sm"
      onClick={e => { if (e.target === e.currentTarget) onClose(); }}
    >
      <div role="dialog" aria-modal="true" aria-labelledby="exercise-progress-title" className="w-full max-w-lg rounded-3xl bg-[var(--color-bg-card)] px-5 pt-4 pb-10 shadow-2xl animate-fade-in">
        {/* Header */}
        <div className="flex items-start justify-between mb-3">
          <div>
            <p className="text-[10px] uppercase tracking-[0.18em] font-semibold text-[var(--color-text-muted)] mb-0.5">
              {t('strength.estimated1RMProgress', 'Estimated 1RM Progress')}
            </p>
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4">
          {gain != null && gain > 0 && (
            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-900/40 text-emerald-400 text-[12px] font-semibold">
              <TrendingUp size={12} />
              {t('strength.gainAcrossPRs', { gain, count: data.length, defaultValue: `+${gain} lbs 1RM across ${data.length} PRs` })}
            </div>
          )}
          {trendPoints.length >= 3 && slope > 0 && projectedOrm != null && (
            <div className="inline-flex items-center gap-1 bg-[var(--color-bg-card)] border border-[#D4AF37]/30 text-[#D4AF37] text-[12px] rounded-full px-2.5 py-1">
              {t('strength.onTrackFor4Weeks', { value: projectedOrm, defaultValue: `On track for ~${projectedOrm} lbs 1RM in 4 weeks` })}
            </div>
          )}
          {trendPoints.length >= 3 && slope <= 0 && (
            <div className="inline-flex items-center gap-1 bg-[var(--color-bg-card)] border border-white/10 text-[var(--color-text-muted)] text-[12px] rounded-full px-2.5 py-1">
              {t('strength.holdingSteady', 'Holding steady — focus on consistency')}
            </div>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-44 gap-2 text-[var(--color-text-subtle)]">
            ...
            <p className="text-[13px] font-medium">{t('strength.noPRHistoryYet', 'No PR history yet')}</p>
            <p className="text-[12px] opacity-70 text-center">{t('strength.hitNewPRToTrack', 'Hit a new personal record to start tracking your progress on this exercise.')}</p>
          </div>
        ) : (
          <FadeIn>
              ...
              {[...data].reverse().slice(0, 4).map((d, i) => (
                <div key={i} className="flex items-center justify-between py-2 border-b border-white/[0.06] last:border-0">
                  ...
                  <span className="text-[13px] font-semibold text-[var(--color-text-primary)]">
                    {t('strength.weightXReps', { weight: d.weight, reps: d.reps, defaultValue: `${d.weight} lbs x ${d.reps}` })}
                    <span className="text-[11px] text-amber-400 font-normal ml-1.5">
                      {t('strength.ormParen', { value: d.orm, defaultValue: `(${d.orm} lbs 1RM)` })}
                    </span>
                  </span>
                </div>
              ))}
