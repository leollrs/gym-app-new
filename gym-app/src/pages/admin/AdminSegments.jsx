...
import { PageHeader, AdminCard, SectionLabel, FadeIn, StatCard, AdminModal, Avatar } from '../../components/admin';
...
export default function AdminSegments() {
  ...
  if (detailSegment) {
    return (
      <SegmentDetail
        ...
        onEdit={() => { setEditModal(detailSegment); setDetailSegment(null); }}
      />
    );
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={t('admin.segments.title', 'Smart Segments')}
        subtitle={t('admin.segments.subtitle', 'Create targeted member lists with dynamic filters')}
        actions={
      ...
      {segments.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {segments.map((seg, i) => (
            <FadeIn key={seg.id} delay={i * 40}>
              <SegmentCard
                segment={seg}
                gymId={gymId}
                onView={() => setDetailSegment(seg)}
      ...
      {/* Suggestions modal */}
      {showSuggestions && (
        <AdminModal isOpen onClose={() => setShowSuggestions(false)} title={t('admin.segments.suggestionsTitle', 'Suggested Segments')} titleIcon={Sparkles}>
  ...
  async function handleDelete(segmentId) {
    ...
    await supabase.from('member_segments').delete().eq('id', segmentId);
    refetch();
  }

  async function handleTogglePin(segment) {
    await supabase.from('member_segments').update({ is_pinned: !segment.is_pinned, updated_at: new Date().toISOString() }).eq('id', segment.id);
    refetch();
  }
}

// ── Segment Card ────────────────────────────────────────────
function SegmentCard({ segment, onView, onEdit, onDelete, onTogglePin, t }) {
  const IconComp = ICON_MAP[segment.icon] || Users;

  return (
    <AdminCard hover onClick={onView} className="relative group">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[20px] font-bold text-[#E5E7EB] tabular-nums">
            {segment._count ?? '—'}
          </span>
...
function PrebuiltCard({ segment, gymId, adminId, onCreated, t }) {
  ...
  return (
    <AdminCard hover className="flex items-center gap-3">
      <div
      ...
      <div className="min-w-0 flex-1">
        <p className="text-[13px] font-bold text-[#E5E7EB] truncate">{segment.nameKey ? t(segment.nameKey, segment.name) : segment.name}</p>
        <p className="text-[11px] text-[#6B7280] truncate">{segment.descriptionKey ? t(segment.descriptionKey, segment.description) : segment.description}</p>
      </div>
      <button
        onClick={handleCreate}
        disabled={saving}
        className="flex-shrink-0 px-3 py-1.5 text-[11px] font-bold text-[#D4AF37] bg-[#D4AF37]/10 hover:bg-[#D4AF37]/20 rounded-lg transition-colors disabled:opacity-50"
      >
        {saving ? '...' : t('admin.segments.addSegment', 'Add')}
      </button>
    </AdminCard>
  );
}
...
// ── Segment Detail View ─────────────────────────────────────
function SegmentDetail({ segment, gymId, adminId, onBack, onEdit }) {
  const { t } = useTranslation('pages');
  ...
  const handleExport = () => {
    exportCSV({
      ...
      columns: [
        ...
        { key: 'created_at', label: 'Joined', format: v => v ? format(new Date(v), 'yyyy-MM-dd') : '' },
        { key: '_lastWorkoutAt', label: 'Last Workout', format: v => v ? format(new Date(v), 'yyyy-MM-dd') : 'Never' },
        { key: '_workoutCount', label: 'Workouts', format: v => v ?? 0 },
        { key: 'current_streak', label: 'Streak', format: v => v ?? 0 },
      ],
      data: filtered,
    });
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        <button onClick={onBack} className="w-9 h-9 rounded-xl bg-white/[0.04] hover:bg-white/[0.06] border border-white/6 flex items-center justify-center text-[#9CA3AF] hover:text-[#E5E7EB] transition-all">
          <ChevronLeft size={18} />
        </button>
        <div className="flex items-center gap-2.5 flex-1 min-w-0">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: `${segment.color}18` }}>
            <IconComp size={16} style={{ color: segment.color }} />
          </div>
          <div className="min-w-0">
        ...
        <span className="text-[20px] font-bold text-[#E5E7EB] tabular-nums flex-shrink-0">{members.length}</span>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button onClick={() => setRefreshKey(k => k + 1)} className="flex items-center gap-1.5 px-3 py-2 text-[12px] font-medium text-[#9CA3AF] hover:text-[#E5E7EB] bg-white/[0.03] hover:bg-white/[0.06] border border-white/6 rounded-lg transition-all">
          <RefreshCw size={13} /> {t('admin.segments.refresh', 'Refresh')}
        </button>
        <button onClick={handleSendMessage} disabled={!filtered.length} className="flex items-center gap-1.5 px-3 py-2 text-[12px] font-medium text-[#9CA3AF] hover:text-[#E5E7EB] bg-white/[0.03] hover:bg-white/[0.06] border border-white/6 rounded-lg transition-all disabled:opacity-40">
          <MessageSquare size={13} /> {t('admin.segments.sendMessage', 'Message All')}
        </button>
        <button onClick={handleExport} disabled={!filtered.length} className="flex items-center gap-1.5 px-3 py-2 text-[12px] font-medium text-[#9CA3AF] hover:text-[#E5E7EB] bg-white/[0.03] hover:bg-white/[0.06] border border-white/6 rounded-lg transition-all disabled:opacity-40">
          <Download size={13} /> {t('admin.segments.export', 'Export CSV')}
        </button>
        ...
        {isLoading ? (
          ...
          <div className="divide-y divide-white/[0.04]">
            {filtered.map(member => {
              ...
              return (
                <div key={member.id} className="flex items-center gap-3 px-4 py-3 hover:bg-white/[0.02] transition-colors">
                  <Avatar name={member.full_name || member.username} size="md" />
                  <div className="min-w-0 flex-1">
                    <p className="text-[13px] font-medium text-[#E5E7EB] truncate">{member.full_name || member.username || 'Unknown'}</p>
                    <p className="text-[11px] text-[#6B7280] truncate">
                      {t('admin.segments.joined', 'Joined')} {member.created_at ? format(new Date(member.created_at), 'MMM d, yyyy') : '—'}
                      {member._lastWorkoutAt && (
                        <> · {t('admin.segments.lastWorkout', 'Last workout')} {format(new Date(member._lastWorkoutAt), 'MMM d')}</>
                      )}
                    </p>
                  </div>
