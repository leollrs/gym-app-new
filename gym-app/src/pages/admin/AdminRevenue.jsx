...
import {
  DollarSign, Coins, ArrowUpDown, ShoppingCart, TrendingUp,
...
export default function AdminRevenue() {
  ...
  useEffect(() => { document.title = 'Admin - Revenue | TuGymPR'; }, []);
  ...
  // ── Queries ────────────────────────────────────────────
  // Points issued/spent from reward_points_log
  ...
  // Purchases with product + member info
  ...
  // Products for punch card analysis
  ...
  // Point flow chart (daily earned vs spent)
  ...
  // Punch card products + completion rates
  ...
  return (
    <div className="px-4 py-6 pb-28 md:pb-12 max-w-[1600px] mx-auto">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-[12px] font-semibold transition-all ${
                period === opt.key
                  ...
                  : 'bg-white/[0.03] text-[#6B7280] border border-white/6 hover:text-[#9CA3AF] hover:bg-white/[0.05]'
              }`}
            >
              {t(`admin.revenue.period.${opt.key}`)}
            </button>
          ))}
        </div>
      </FadeIn>
        ...
        <AdminCard className="mb-4">
          ...
          {isLoading ? (
            ...
            <div className="mt-3 space-y-0">
              {/* Header */}
              <div className="grid grid-cols-[1fr_80px_90px] gap-2 px-3 py-2 text-[10px] font-semibold text-[#6B7280] uppercase tracking-wider">
              ...
              {topProducts.map((p) => (
                <div
                  key={p.id}
                  className="grid grid-cols-[1fr_80px_90px] gap-2 px-3 py-2.5 rounded-lg hover:bg-white/[0.02] transition-colors border-b border-white/[0.03] last:border-0"
                >
      ...
      {punchCardData.length > 0 && (
        <FadeIn delay={360}>
          <AdminCard className="mb-4">
            <SectionLabel>{t('admin.revenue.punchCards', 'Punch Card Usage')}</SectionLabel>
      ...
      <FadeIn delay={420}>
        <AdminCard>
          <SectionLabel>{t('admin.revenue.recentRedemptions', 'Recent Redemptions')}</SectionLabel>
          {isLoading ? (
                ...
                >
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <p className="text-[10px] text-[#6B7280]">
                      {format(parseISO(r.created_at), 'MMM dd')}
                    </p>
                  </div>
                </div>
              ))}
