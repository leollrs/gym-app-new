  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center justify-between">
        ...
        <p className="text-[13px] font-black text-[#E5E7EB]">
          {value ?? '—'}
          <span className="text-[11px] font-medium ml-0.5 text-[#6B7280]">{unit}</span>
        </p>
      </div>
  ...
  return (
    <div className="mx-auto w-full max-w-[720px] md:max-w-4xl px-4 md:px-6 pt-6 pb-28 md:pb-12 animate-fade-in">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-[20px] font-bold text-[#E5E7EB]">Nutrition</h1>
          <p className="text-[12px] text-[#9CA3AF]">Daily targets &amp; check-ins</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <>
          {/* ── Daily targets card ─────────────────────────────────────────── */}
          <div className="bg-[#0F172A] rounded-[14px] border border-white/8 p-5 mb-4">
            <div className="flex items-center justify-between mb-4">
              <p className="text-[14px] font-semibold text-[#E5E7EB]">Daily Targets</p>
              <button
                onClick={openEdit}
                className="flex items-center gap-1.5 text-[12px] font-semibold px-3 py-1.5 rounded-xl bg-[#D4AF37]/10 text-[#D4AF37] border border-[#D4AF37]/20"
              >
            ...
            {targets ? (
              <div className="space-y-3.5">
                ...
                {onboarding?.primary_goal && (
                  <p className="text-[11px] pt-1 text-[#9CA3AF]">
                    Optimized for <span className="text-[#D4AF37]">{GOAL_LABELS[onboarding.primary_goal] ?? onboarding.primary_goal}</span>
                  </p>
                )}
              </div>
            ) : (
              <div className="py-4 text-center">
                <p className="text-[13px] mb-1 text-[#9CA3AF]">No targets set yet</p>
                ...
                <div className="p-5 space-y-4 overflow-y-auto flex-1 min-h-0">
                  {bodyweight && (
                    <button
                      ...
                      className="w-full py-2.5 rounded-xl text-[13px] font-bold border border-[#D4AF37]/30 text-[#D4AF37] bg-[#D4AF37]/6 transition-colors hover:bg-[#D4AF37]/10"
                    >
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                <Flame
                  ...
                  strokeWidth={2}
                />
                <p className="text-[13px] font-bold" style={{ color: todayLog?.hit_calories ? '#D4AF37' : '#9CA3AF' }}>
                  {todayLog?.hit_calories ? 'Hit Calories ✓' : 'Calories?'}
                </p>
                {targets?.daily_calories && (
                  <p className="text-[11px] text-[#9CA3AF]">{targets.daily_calories} kcal</p>
                )}
              </button>
              ...
              >
                <Check
                  ...
                  strokeWidth={2.5}
                />
                <p className="text-[13px] font-bold" style={{ color: todayLog?.hit_protein ? '#60A5FA' : '#9CA3AF' }}>
                  {todayLog?.hit_protein ? 'Hit Protein ✓' : 'Protein?'}
                </p>
                {targets?.daily_protein_g && (
                  <p className="text-[11px] text-[#9CA3AF]">{targets.daily_protein_g}g protein</p>
                )}
              </button>
            </div>
          </div>
          ...
          <div className="bg-[#0F172A] rounded-[14px] border border-white/8 p-5 mb-4">
            <div className="flex items-center justify-between mb-4">
              <p className="text-[14px] font-semibold text-[#E5E7EB]">Consistency</p>
              <div className="flex items-center gap-2">
                <Flame size={14} className="text-[#D4AF37]" />
                <span className="text-[13px] font-bold text-[#D4AF37]">{streak} day streak</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 mt-3">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] text-[#9CA3AF]">Both hit</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-3 h-3 rounded-[3px] bg-[#D4AF37]/40" />
                <span className="text-[10px] text-[#9CA3AF]">Partial</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-3 h-3 rounded-[3px] bg-white/4" />
                <span className="text-[10px] text-[#9CA3AF]">Missed</span>
              </div>
            </div>
          </div>
        </>
