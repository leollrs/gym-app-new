...
function bodyReducer(state, action) {
  switch (action.type) {
    ...
    case 'TOGGLE_MONTHLY_REPORT':
      return { ...state, showMonthlyReport: action.payload };
    default:
      return state;
  }
}
...
export default function ProgressBody() {
  ...
  const {
    ...
    weightError, latestMeasurements, showMeasurements, showMonthlyReport, loading,
  } = state;
  ...
  const handleLogWeight = async () => {
    ...
    if (!w || w <= 0) {
      dispatch({ type: 'SET_WEIGHT_ERROR', payload: 'Enter a valid weight' });
      return;
    }
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <button
          onClick={() => dispatch({ type: 'TOGGLE_MONTHLY_REPORT', payload: true })}
      ...
      {showMonthlyReport && (
        <MonthlyProgressReport
          ...
          onClose={() => dispatch({ type: 'TOGGLE_MONTHLY_REPORT', payload: false })}
        />
      )}
    </div>
  );
