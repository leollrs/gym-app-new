...
const FITNESS_LEVELS = [
  { value: 'beginner',     label: 'Beginner',     icon: Sprout,  desc: 'Less than 1 year of consistent training' },
  { value: 'intermediate', label: 'Intermediate', icon: Zap,     desc: '1–3 years of consistent training' },
  { value: 'advanced',     label: 'Advanced',     icon: Trophy,  desc: '3+ years, comfortable with complex movements' },
];

const GOALS = [
  { value: 'muscle_gain',     label: 'Build Muscle',   icon: Dumbbell,  desc: 'Maximize hypertrophy and size' },
  { value: 'fat_loss',        label: 'Lose Fat',        icon: Flame,     desc: 'Burn fat while preserving muscle' },
  { value: 'strength',        label: 'Get Stronger',    icon: Dumbbell,  desc: 'Increase 1RMs and raw strength' },
  { value: 'endurance',       label: 'Build Endurance', icon: Activity,  desc: 'Improve stamina and conditioning' },
  { value: 'general_fitness', label: 'General Fitness', icon: Sparkles,  desc: 'Stay active, healthy and consistent' },
];
...
const Profile = () => {
  ...
  const saveGoals = async () => {
    ...
    try {
      ...
      if (error) {
        logger.error('saveGoals error:', error);
  ...
  // ── Render ──────────────────────────────────────────────────────────────────
  return (
    <div className="min-h-screen bg-[#05070B] pb-28 md:pb-12" data-tour="tour-profile-page">
      ...
      <div className="rounded-2xl bg-[#0F172A] border border-white/8 mb-6 overflow-hidden">
        ...
        <div className="flex items-start justify-between p-6 pb-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="w-10 h-10 flex items-center justify-center rounded-xl transition-colors hover:opacity-80 active:scale-95 bg-white/[0.06] border border-white/10 text-[#9CA3AF]"
              title="Settings"
            >
              <Settings size={18} />
            </button>
        ...
        {joinDate && (
          <p className="text-[12px] flex items-center gap-1.5 px-6 pb-2 text-[#6B7280]">
        ...
        <div className="flex items-center gap-2 px-6 pb-4">
          ...
          <span className="text-[13px] font-bold text-[#E5E7EB]">{t('profile.level')} {level}</span>
          <span
        ...
        <div className="animate-fade-in">
          {editingGoals && goalsDraft ? (
            ...
            <div className="flex flex-col gap-6 pb-2">
              ...
              {/* Fitness Level */}
              <div>
                <h3 className="text-[11px] font-semibold text-[#6B7280] uppercase tracking-widest mb-3">{t('profile.fitnessLevel')}</h3>
              ...
              {/* Primary Goal */}
              <div>
                <h3 className="text-[11px] font-semibold text-[#6B7280] uppercase tracking-widest mb-3">{t('profile.primaryGoal')}</h3>
                <div className="flex flex-col gap-3">
                  {GOALS.map(g => (
              ...
              {/* Training Days */}
              <div>
                <h3 className="text-[11px] font-semibold text-[#6B7280] uppercase tracking-widest mb-3">{t('profile.daysPerWeek')}</h3>
              ...
              {/* Equipment */}
              <div>
                <h3 className="text-[11px] font-semibold text-[#6B7280] uppercase tracking-widest mb-3">{t('profile.availableEquipment')}</h3>
              ...
              {/* Injuries */}
              <div>
                <h3 className="text-[11px] font-semibold text-[#6B7280] uppercase tracking-widest mb-3">{t('profile.injuriesLimitations')}</h3>
                ...
                {(goalsDraft.injury_areas ?? []).length === 0 && (
                  <p className="text-[11px] text-[#6B7280]">{t('profile.nothingSelectedAllExercises')}</p>
                )}
              </div>
                ...
                <>
                  ...
                  <div className="rounded-2xl bg-[#0F172A] border border-white/8 p-5">
                    ...
                    {(() => {
                      ...
                      ) : <p className="text-[#9CA3AF]">{t('profile.notSet')}</p>;
                    })()}
                  </div>
                  ...
                  <div className="rounded-2xl bg-[#0F172A] border border-white/8 p-5">
                    ...
                    {(() => {
                      ...
                      ) : <p className="text-[14px] text-[#9CA3AF]">{t('profile.noneNoted')}</p>;
                    })()}
                  </div>
      ...
      {showGymInfo && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowGymInfo(false)}>
          <div className="w-full max-w-md mx-4 rounded-[24px] bg-[#0A0F1A] border border-white/[0.06] shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="relative flex justify-center pt-4 pb-3">
              <div className="w-8 h-[3px] rounded-full bg-white/[0.08]" />
              <button onClick={() => setShowGymInfo(false)} className="absolute right-4 top-3 w-8 h-8 rounded-full bg-white/[0.04] flex items-center justify-center text-[#6B7280]">
                <X size={16} />
              </button>
            </div>
            <div className="px-6 pb-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="text-[17px] font-bold text-[#E5E7EB]">{gymInfo?.name || gymName}</h3>
                  <p className="text-[12px] text-[#6B7280]">{t('profile.gymInfo')}</p>
                </div>
              </div>
              <div className="space-y-3">
                {gymInfo?.address && (
                  <div className="flex items-start gap-3 text-[13px]">
                    <span className="text-[#4B5563] w-20 shrink-0 font-medium">{t('profile.address')}</span>
                    <span className="text-[#9CA3AF]">{gymInfo.address}</span>
                  </div>
                )}
                {gymInfo?.phone && (
                  <div className="flex items-start gap-3 text-[13px]">
                    <span className="text-[#4B5563] w-20 shrink-0 font-medium">{t('profile.phone')}</span>
                    <span className="text-[#9CA3AF]">{gymInfo.phone}</span>
                  </div>
                )}
                {gymInfo?.email && (
                  <div className="flex items-start gap-3 text-[13px]">
                    <span className="text-[#4B5563] w-20 shrink-0 font-medium">{t('profile.email')}</span>
                    <span className="text-[#9CA3AF]">{gymInfo.email}</span>
                  </div>
                )}
                {gymInfo?.opening_hours && (
                  <div className="flex items-start gap-3 text-[13px]">
                    <span className="text-[#4B5563] w-20 shrink-0 font-medium">{t('profile.hours')}</span>
                    <span className="text-[#9CA3AF] whitespace-pre-line">{gymInfo.opening_hours}</span>
                  </div>
                )}
                {gymInfo?.website && (
                  <div className="flex items-start gap-3 text-[13px]">
                    <span className="text-[#4B5563] w-20 shrink-0 font-medium">{t('profile.website')}</span>
                    <span className="text-[#9CA3AF]">{gymInfo.website}</span>
                  </div>
                )}
                {!gymInfo?.address && !gymInfo?.phone && !gymInfo?.opening_hours && (
                  <p className="text-[13px] text-[#4B5563] text-center py-4">{t('profile.noGymInfoYet')}</p>
                )}
              </div>
            </div>
          </div>
