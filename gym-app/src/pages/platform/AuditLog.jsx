...
import {
  ...
  ChevronDown, Search, Filter, Loader2, AlertTriangle, KeyRound, Mail,
  MessageSquare, Megaphone, Flag, Upload, Download, ScanLine, Gift,
  Printer, Tv, Trash2, UserX, UserPlus, Wrench,
} from 'lucide-react';
...
// Real actions only — every key below is written by logAdminAction(...) calls
// in src/ (grepped 2026-06-11) or by RPC-internal inserts. Anything else falls
// through to configForAction()'s prefix heuristics.
const ACTION_CONFIG = {
  // Platform tier
  create_gym:                  { icon: Building2, color: 'emerald', labelKey: 'platform.audit.actions.createGym' },
  pause_gym:                   { icon: Building2, color: 'amber',   labelKey: 'platform.audit.actions.pauseGym' },
  reactivate_gym:              { icon: Building2, color: 'emerald', labelKey: 'platform.audit.actions.reactivateGym' },
  permanently_deactivate_gym:  { icon: Building2, color: 'red',     labelKey: 'platform.audit.actions.permanentlyDeactivateGym' },
  gym_import:                  { icon: Upload,    color: 'purple',  labelKey: 'platform.audit.actions.gymImport' },
  super_admin_schedule_deletion: { icon: Shield,  color: 'red',     labelKey: 'platform.audit.actions.superAdminScheduleDeletion' },
  super_admin_cancel_deletion: { icon: Shield,    color: 'emerald', labelKey: 'platform.audit.actions.superAdminCancelDeletion' },
  super_admin_delete_gym_now:  { icon: Trash2,    color: 'red',     labelKey: 'platform.audit.actions.superAdminDeleteGymNow' },
  super_admin_export:          { icon: Download,  color: 'gold',    labelKey: 'platform.audit.actions.superAdminExport' },
  toggle_feature_flag:         { icon: Settings,  color: 'amber',   labelKey: 'platform.audit.actions.toggleFeatureFlag' },
  toggle_maintenance:          { icon: Wrench,    color: 'red',     labelKey: 'platform.audit.actions.toggleMaintenance' },
  toggle_email_flag:           { icon: Mail,      color: 'amber',   labelKey: 'platform.audit.actions.toggleEmailFlag' },
  save_gym_defaults:           { icon: Settings,  color: 'blue',    labelKey: 'platform.audit.actions.saveGymDefaults' },
  // Member support
  change_role:                 { icon: UserCog,   color: 'purple',  labelKey: 'platform.audit.actions.changeRole' },
  change_status:               { icon: Shield,    color: 'amber',   labelKey: 'platform.audit.actions.changeStatus' },
  reset_password:              { icon: KeyRound,  color: 'amber',   labelKey: 'platform.audit.actions.resetPassword' },
  deactivate_member:           { icon: UserX,     color: 'red',     labelKey: 'platform.audit.actions.deactivateMember' },
  ...
  block_user:                  { icon: Shield,    color: 'red',     labelKey: 'platform.audit.actions.blockUser' },
  unblock_user:                { icon: Shield,    color: 'emerald', labelKey: 'platform.audit.actions.unblockUser' },
  bulk_freeze:                 { icon: Snowflake, color: 'amber',   labelKey: 'platform.audit.actions.bulkFreeze' },
  bulk_unfreeze:               { icon: Snowflake, color: 'emerald', labelKey: 'platform.audit.actions.bulkUnfreeze' },
  bulk_export:                 { icon: Download,  color: 'gold',    labelKey: 'platform.audit.actions.bulkExport' },
  export_member:               { icon: Download,  color: 'gold',    labelKey: 'platform.audit.actions.exportMember' },
  // Communications
  send_message:                { icon: MessageSquare, color: 'blue',   labelKey: 'platform.audit.actions.sendMessage' },
  quick_message:               { icon: MessageSquare, color: 'blue',   labelKey: 'platform.audit.actions.quickMessage' },
  bulk_message:                { icon: MessageSquare, color: 'blue',   labelKey: 'platform.audit.actions.bulkMessage' },
  send_sms:                    { icon: MessageSquare, color: 'amber',  labelKey: 'platform.audit.actions.sendSms' },
  send_email:                  { icon: Mail,          color: 'blue',   labelKey: 'platform.audit.actions.sendEmail' },
  send_winback:                { icon: MessageSquare, color: 'purple', labelKey: 'platform.audit.actions.sendWinback' },
  send_followup:               { icon: MessageSquare, color: 'blue',   labelKey: 'platform.audit.actions.sendFollowup' },
  outreach_send:               { icon: MessageSquare, color: 'purple', labelKey: 'platform.audit.actions.outreachSend' },
  set_outcome:                 { icon: Settings,      color: 'emerald', labelKey: 'platform.audit.actions.setOutcome' },
  // Moderation
  moderation:                  { icon: Flag,      color: 'amber',   labelKey: 'platform.audit.actions.moderation' },
  // Settings
  ...
  create_announcement:         { icon: Megaphone, color: 'emerald', labelKey: 'platform.audit.actions.createAnnouncement' },
  ...
  // Store / rewards
  create_product:              { icon: Gift,      color: 'emerald', labelKey: 'platform.audit.actions.createProduct' },
  update_product:              { icon: Gift,      color: 'blue',    labelKey: 'platform.audit.actions.updateProduct' },
  toggle_product:              { icon: Gift,      color: 'amber',   labelKey: 'platform.audit.actions.toggleProduct' },
  delete_product:              { icon: Gift,      color: 'red',     labelKey: 'platform.audit.actions.deleteProduct' },
  activate_reward:             { icon: Gift,      color: 'emerald', labelKey: 'platform.audit.actions.activateReward' },
  deactivate_reward:           { icon: Gift,      color: 'amber',   labelKey: 'platform.audit.actions.deactivateReward' },
  delete_reward:               { icon: Gift,      color: 'red',     labelKey: 'platform.audit.actions.deleteReward' },
  claim_reward:                { icon: Gift,      color: 'gold',    labelKey: 'platform.audit.actions.claimReward' },
  redeem_earned_reward:        { icon: Gift,      color: 'gold',    labelKey: 'platform.audit.actions.redeemEarnedReward' },
  // Scans
  checkin_scan:                { icon: ScanLine,  color: 'emerald', labelKey: 'platform.audit.actions.checkinScan' },
  purchase_scan:               { icon: ScanLine,  color: 'gold',    labelKey: 'platform.audit.actions.purchaseScan' },
  referral_scan:               { icon: ScanLine,  color: 'purple',  labelKey: 'platform.audit.actions.referralScan' },
};

// Prefix fallbacks for action families (tv_*, print_*, super_admin_*…) and
// unknown-but-shaped actions, so real entries never get a misleading icon.
function configForAction(action = '') {
  if (ACTION_CONFIG[action]) return ACTION_CONFIG[action];
  if (action.startsWith('tv_'))          return { icon: Tv,       color: 'blue' };
  if (action.startsWith('print_'))       return { icon: Printer,  color: 'blue' };
  if (action.startsWith('super_admin_')) return { icon: Shield,   color: 'gold' };
  if (action.startsWith('delete_'))      return { icon: Trash2,   color: 'red' };
  if (action.startsWith('create_'))      return { icon: Settings, color: 'emerald' };
  if (action.startsWith('send_') || action.startsWith('bulk_')) return { icon: MessageSquare, color: 'blue' };
  if (action.startsWith('update_') || action.startsWith('toggle_') || action.startsWith('settings_')) return { icon: Settings, color: 'blue' };
  return { icon: Settings, color: 'blue' };
}
...
// Filter options = actions that are actually produced (verified against every
// logAdminAction call in src/). Values ending in '*' filter by prefix.
const ACTION_TYPE_KEYS = [
  { labelKey: 'platform.audit.filter.allActions',                    fallback: 'All Actions',          value: 'all' },
  // Platform tier
  { labelKey: 'platform.audit.actions.createGym',                    fallback: 'Gym Created',          value: 'create_gym' },
  { labelKey: 'platform.audit.actions.pauseGym',                     fallback: 'Gym Paused',           value: 'pause_gym' },
  { labelKey: 'platform.audit.actions.reactivateGym',                fallback: 'Gym Reactivated',      value: 'reactivate_gym' },
  { labelKey: 'platform.audit.actions.permanentlyDeactivateGym',     fallback: 'Gym Deactivated',      value: 'permanently_deactivate_gym' },
  { labelKey: 'platform.audit.actions.gymImport',                    fallback: 'Gym Import',           value: 'gym_import' },
  { labelKey: 'platform.audit.filter.superAdminGroup',               fallback: 'Super-admin actions',  value: 'super_admin_*' },
  { labelKey: 'platform.audit.actions.toggleFeatureFlag',            fallback: 'Feature Flag Toggled', value: 'toggle_feature_flag' },
  { labelKey: 'platform.audit.actions.toggleMaintenance',            fallback: 'Maintenance Toggled',  value: 'toggle_maintenance' },
  // Member support
  { labelKey: 'platform.audit.actions.changeRole',                   fallback: 'Role Changed',         value: 'change_role' },
  { labelKey: 'platform.audit.actions.changeStatus',                 fallback: 'Status Changed',       value: 'change_status' },
  { labelKey: 'platform.audit.actions.resetPassword',                fallback: 'Password Reset',       value: 'reset_password' },
  { labelKey: 'platform.audit.actions.deactivateMember',             fallback: 'Member Deactivated',   value: 'deactivate_member' },
  { labelKey: 'platform.audit.actions.generateInviteCode',           fallback: 'Invite Code Generated', value: 'generate_invite_code' },
  // Gym-admin high frequency
  { labelKey: 'platform.audit.actions.moderation',                   fallback: 'Moderation',           value: 'moderation' },
  ...
  { labelKey: 'platform.audit.filter.tvGroup',                       fallback: 'TV display actions',   value: 'tv_*' },
  { labelKey: 'platform.audit.filter.printGroup',                    fallback: 'Print-card actions',   value: 'print_*' },
];
...
function buildDescription(action, metadata, t) {
  ...
  switch (action) {
    ...
    case 'gym_import':
      return t('platform.audit.desc.gymImport', { count: metadata?.count || metadata?.imported || '?', defaultValue: 'imported {{count}} members' });
    case 'super_admin_schedule_deletion':
      return t('platform.audit.desc.scheduleDeletion', { name: metadata?.gym_name || unknown(), defaultValue: 'scheduled deletion of gym "{{name}}"' });
    case 'super_admin_cancel_deletion':
      return t('platform.audit.desc.cancelDeletion', { name: metadata?.gym_name || unknown(), defaultValue: 'cancelled scheduled deletion of gym "{{name}}"' });
    case 'super_admin_delete_gym_now':
      return t('platform.audit.desc.deleteGymNow', { name: metadata?.gym_name || unknown(), defaultValue: 'permanently deleted gym "{{name}}"' });
    case 'super_admin_export':
      return t('platform.audit.desc.superAdminExport', { what: metadata?.what || metadata?.type || 'data', defaultValue: 'exported {{what}}' });
    ...
    case 'toggle_maintenance':
      return t('platform.audit.desc.toggleMaintenance', { state: metadata?.enabled ? t('platform.audit.on', 'on') : t('platform.audit.off', 'off'), defaultValue: 'turned {{state}} maintenance mode' });
    ...
    case 'change_status':
      return t('platform.audit.desc.statusChanged', { target, oldStatus: metadata?.from || '?', newStatus: metadata?.to || '?', defaultValue: 'changed {{target}} status from {{oldStatus}} to {{newStatus}}' }) + banReason;
    case 'deactivate_member':
      return t('platform.audit.desc.memberDeactivated', { target, defaultValue: 'deactivated {{target}}' });
    case 'delete_account':
      return t('platform.audit.desc.deleteAccount', { target, defaultValue: 'deleted account of {{target}}' });
    case 'reset_password':
    ...
    case 'generate_invite_code':
      return t('platform.audit.desc.generateInviteCode', { target, defaultValue: 'generated a new invite code for {{target}}' });
    case 'invite_member':
    ...
    case 'block_user':
      return t('platform.audit.desc.blockUser', { target, defaultValue: 'blocked {{target}}' });
    case 'unblock_user':
      return t('platform.audit.desc.unblockUser', { target, defaultValue: 'unblocked {{target}}' });
    case 'bulk_unfreeze':
      return t('platform.audit.desc.bulkUnfreeze', { count: metadata?.count || '?', defaultValue: 'unfroze {{count}} memberships' });
    ...
    case 'outreach_send':
      return t('platform.audit.desc.outreachSend', { target, defaultValue: 'sent outreach to {{target}}' });
    case 'bulk_freeze':
    ...
    case 'activate_reward':
      return t('platform.audit.desc.activateReward', { name: metadata?.name || unknown(), defaultValue: 'activated reward "{{name}}"' });
    case 'deactivate_reward':
      return t('platform.audit.desc.deactivateReward', { name: metadata?.name || unknown(), defaultValue: 'deactivated reward "{{name}}"' });
    case 'delete_reward':
    ...
    case 'redeem_earned_reward':
      return t('platform.audit.desc.redeemEarnedReward', { target, defaultValue: 'redeemed an earned reward for {{target}}' });
    ...
    // Moderation (the single real action key — details say what happened)
    case 'moderation':
      return t('platform.audit.desc.moderation', { action: metadata?.moderation_action || metadata?.action || metadata?.outcome || 'reviewed', defaultValue: 'moderated content: {{action}}' });
...
function LogEntry({ entry, t, dateFnsLocale }) {
  ...
  const config = configForAction(entry.action);
...
export default function AuditLog() {
  ...
  // true = server-side exact distinct count; false = computed from loaded rows
  const [actorCountExact, setActorCountExact] = useState(false);
  ...
  // actionType values ending in '*' are prefix groups (tv_*, print_*…).
  const applyFilters = useCallback((query) => {
    ...
    if (actionType !== 'all') {
      if (actionType.endsWith('*')) {
        query = query.like('action', `${actionType.slice(0, -1)}%`);
      } else {
  ...
  const fetchEntries = useCallback(async (offset = 0, append = false) => {
    ...
    // Distinct actor count: exact server-side count via RPC (0543). The old
    // approach selected every actor_id unranged — silently capped at 1000
    // rows. Falls back to loaded-rows math (qualified in the tile) if the
    // RPC isn't available yet.
    if (!append) {
      const now = new Date();
      let pStart = null;
      if (dateRange === '24h') pStart = subHours(now, 24).toISOString();
      else if (dateRange === '7d') pStart = subDays(now, 7).toISOString();
      else if (dateRange === '30d') pStart = subDays(now, 30).toISOString();

      const rpcParams = {
        p_start: pStart,
        p_end: null,
        p_action: actionType !== 'all' && !actionType.endsWith('*') ? actionType : null,
        p_action_prefix: actionType !== 'all' && actionType.endsWith('*') ? actionType.slice(0, -1) : null,
        p_gym_id: gymFilter !== 'all' ? gymFilter : null,
        p_search: debouncedSearch.trim() ? debouncedSearch.trim().replace(/[%_\\,()."']/g, '') : null,
      };

      const { data: actorCount, error: actorError } = await supabase.rpc('platform_audit_actor_count', rpcParams);
      if (!actorError && typeof actorCount === 'number') {
        setUniqueActors(actorCount);
        setActorCountExact(true);
      } else {
        if (actorError) logger.error('Error fetching audit actor count:', actorError);
        setUniqueActors([...new Set(results.map(r => r.actor_id).filter(Boolean))].length);
        setActorCountExact(false);
      }
    }
  }, [applyFilters, dateRange, actionType, gymFilter, debouncedSearch]);
  ...
  return (
    <div className="px-4 py-6 max-w-[480px] mx-auto md:max-w-4xl pb-28 md:pb-12">
      ...
      {/* Summary strip — Total Actions + Active Actors are server counts
          (actor count falls back to loaded rows pre-0543, qualified below);
          Top Action is always computed from the loaded page(s). */}
      {!loading && !fetchError && (totalCount > 0 || entries.length > 0) && (
        <div className="grid grid-cols-3 md:grid-cols-3 gap-2.5 mb-6">
          ...
          <div className="bg-[#0F172A] border border-white/6 rounded-xl p-3.5">
            ...
            <p className="text-[10px] text-[#6B7280] mt-0.5">
              {t('platform.audit.activeActors', 'Active Actors')}
              {!actorCountExact && entries.length < totalCount ? ` (${t('platform.audit.fromLoaded', 'of loaded')})` : ''}
            </p>
          </div>
          <div className="bg-[#0F172A] border border-white/6 rounded-xl p-3.5">
            <p className="text-[18px] font-bold text-[#E5E7EB] tabular-nums truncate">
              {(() => {
                ...
                if (!top) return '—';
                const labelKey = ACTION_CONFIG[top[0]]?.labelKey;
                return labelKey ? t(labelKey, top[0].replace(/_/g, ' ')) : top[0].replace(/_/g, ' ');
              })()}
            </p>
            <p className="text-[10px] text-[#6B7280] mt-0.5">{t('platform.audit.topAction', 'Top Action')}{entries.length < totalCount ? ` (${t('platform.audit.fromLoaded', 'of loaded')})` : ''}</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-[#0F172A] border border-white/6 rounded-xl p-4 overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-[14px] text-red-400">{t('platform.audit.fetchFailed', 'Could not load the audit log')}</p>
            <p className="text-[12px] text-[#6B7280]/60 mt-1">
              {t('platform.audit.fetchFailedHint', 'The query failed — this is not an empty log.')}
            </p>
            <button
