...
export async function getUserPoints(userId) {
  ...
  if (!data) {
    ...
    if (upsertErr) {
      console.error('getUserPoints upsert error:', upsertErr);
      return { total_points: 0, lifetime_points: 0 };
    }
    return newRow;
  }
...
export async function addPoints(userId, gymId, action, points, description) {
  ...
  if (logErr) {
    console.error('addPoints log error:', logErr);
    return null;
  }
  ...
  if (updateErr) {
    console.error('addPoints update error:', updateErr);
    return null;
  }
