-- =============================================================
-- Fix: Remove ::UUID casts on exercise_id in complete_workout RPC.
-- Also creates missing reward_points tables.
-- Migration: 0082_fix_exercise_id_cast.sql
-- =============================================================

-- ── Create missing reward tables ────────────────────────────
...
CREATE INDEX IF NOT EXISTS idx_reward_points_log_profile
  ON reward_points_log(profile_id, created_at DESC);
...
CREATE POLICY "reward_points_log_own" ON reward_points_log
  FOR SELECT USING (profile_id = auth.uid());
CREATE POLICY "reward_points_log_admin" ON reward_points_log
  FOR ALL USING (
...
BEGIN
  ...
  LOOP
    INSERT INTO session_exercises (
      ...
      v_session_id,
      v_ex->>'exercise_id',           -- TEXT, not UUID
      v_ex->>'name',
      (v_ex->>'position')::INT
    )
  ...
  IF p_payload->'session_prs' IS NOT NULL AND json_array_length(p_payload->'session_prs') > 0 THEN
    ...
    LOOP
      ...
      INSERT INTO personal_records (
        ...
        v_user_id, v_gym_id, v_pr->>'exercise_id',   -- TEXT, not UUID
...
GRANT EXECUTE ON FUNCTION public.complete_workout(JSON) TO authenticated;
