-- =============================================================
-- FIX: redeem_reward — skip UUID cast for string-based reward IDs
--
-- Problem: hardcoded catalog uses string IDs ('smoothie', 'guest',
-- etc.) which fail when cast to UUID in the gym_rewards lookup.
--
-- Fix: wrap the gym_rewards lookup in an exception handler so
-- non-UUID reward IDs gracefully skip server-side cost validation
-- (the client-sent cost is still validated as positive).
-- =============================================================
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'gym_rewards') THEN
    ...
    EXCEPTION WHEN invalid_text_representation THEN
      -- p_reward_id is not a valid UUID (e.g. 'smoothie') — skip lookup
      NULL;
    END;
  END IF;
