-- ============================================================
-- 0353 — Fix streak count + break-date semantics (round 2)
-- ============================================================
-- Two bugs surfaced after 0352:
--
-- Bug A — "Última perdida" shows a TRAINED day
-- 0352 set `streak_broken_at = last_activity_date` (the last trained day
-- before the gap). But "última perdida" / "last lost" should answer
-- "when did the streak end?" — that's the first UNPROTECTED gap day,
-- not the last trained day. We now capture the first gap day that
-- failed the protection check and use that as the break date.
--
-- Bug B — Streak only counts WORKOUT days, not protected gap days
-- The streak was incrementing by +1 per workout completion. So a user
-- who trains Mon + Wed (with Tuesday as a protected rest day) ended
-- up with streak=2 instead of streak=3. Same problem during the daily
-- cron sweep — `check_daily_streaks` advances `last_activity_date`
-- through protected days but never adds the days to `current_streak_days`.
-- Fix: every protected day in a gap increments the streak by 1.
--
-- complete_workout: streak += (today - last_activity_date)
-- check_daily_streaks: streak += (last_protected - last_activity_date)
-- ============================================================
    ...
    ELSE
      ...
      WHILE v_gap_date < v_today AND NOT v_streak_broken LOOP
        IF NOT public._streak_gap_day_protected(
          ...
          v_first_unprotected := v_gap_date;  -- record the day they ACTUALLY lost
        END IF;
      ...
      IF v_streak_broken THEN
        v_new_streak  := 1;
        v_new_longest := v_existing_streak.longest_streak_days;
        -- "Última perdida" = the first unprotected gap day. If the user
        -- trained Mon and skipped Tue, "última perdida" is Tue (the day
        -- they actually missed), not Mon (the last trained day).
        v_streak_broken_at := v_first_unprotected::TIMESTAMPTZ;
      ELSE
        -- Gap was fully protected. Streak grows by ALL the days in the
        -- gap (rest/closed/frozen), not just by 1. So "trained Mon, Wed"
        -- with Tuesday rest = +2 (Tue + Wed), reaching streak = 3.
...
-- Same patches:
--   * record first_unprotected day as streak_broken_at (not now())
--   * when gap is protected, ADD the protected days to the streak count
--     (and bump longest_streak_days)
...
BEGIN
  FOR v_user IN
    SELECT sc.profile_id, sc.gym_id, sc.current_streak_days, sc.longest_streak_days, sc.last_activity_date,
  ...
  LOOP
    ...
    IF v_streak_broken THEN
      UPDATE streak_cache SET
        ...
        streak_broken_at    = v_first_unprotected::TIMESTAMPTZ,
        updated_at          = now()
      WHERE profile_id = v_user.profile_id;
      v_broken_count := v_broken_count + 1;
    ELSE
      IF v_last_protected_date > v_user.last_activity_date THEN
        v_advance_days := v_last_protected_date - v_user.last_activity_date;
        UPDATE streak_cache SET
          last_activity_date  = v_last_protected_date,
          current_streak_days = current_streak_days + v_advance_days,
          longest_streak_days = GREATEST(longest_streak_days, current_streak_days + v_advance_days),
          updated_at          = now()
        WHERE profile_id = v_user.profile_id;
      END IF;
