-- =============================================================
-- 0455 — Upcoming print cards: surface already-queued cards
--
-- Before this, get_upcoming_print_cards (0416) EXCLUDED any member who
-- already had a pending/printed card for the occasion (NOT EXISTS guard).
-- So the moment an owner hit "Print early" (materialize → pending) or
-- printed a card, it vanished from "Próximas tarjetas" — even though the
-- milestone/birthday is still days away. Owners read that as "the card
-- disappeared and isn't in any tab."
--
-- New behaviour: the milestone / tenure / birthday branches no longer
-- exclude an already-queued member. Instead they LEFT JOIN the active
-- (pending|printed) card and return its id + status, so the panel can keep
-- showing the row for the whole week with an "en cola / impresa" marker and
-- a "Ver" action (open that card) instead of "Imprimir ya".
--
-- The habit_9in6 branch keeps its original rolling-window dedup unchanged
-- (that guard is about not re-gifting a habit card within N days, not about
-- the lifecycle of a single upcoming card) — its card_id/card_status return
-- NULL.
--
-- Return-type change (two new columns) → must DROP before CREATE.
-- =============================================================
...
RETURNS TABLE (
  ...
  current_value    INT,
  card_id          UUID,            -- existing active card (NULL = not queued yet)
  card_status      TEXT             -- 'pending' | 'printed' | NULL
)
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  -- ── habit_9in6 approaching — UNCHANGED (keeps its rolling dedup) ──
  SELECT
