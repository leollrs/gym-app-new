-- =============================================================
-- 0258 — Fix program_enrollment_id + return completed_sets
--
-- 1. complete_workout now auto-detects the user's active program
--    enrollment (via routine_id → program_week_days → program_weeks
--    → user_enrolled_programs) and sets program_enrollment_id on
--    the workout_sessions row.
--
-- 2. Returns completed_sets in the JSON response so the frontend
--    can use the server-computed value instead of stale closures.
-- =============================================================
...
DECLARE
  ...
  -- server-side set count (computed from payload)
  v_actual_sets      INT := 0;
BEGIN
  ...
  -- ── Look up active program enrollment for this routine ─────────────────────
  IF v_routine_id IS NOT NULL THEN
    SELECT uep.id INTO v_enrollment_id
      FROM user_enrolled_programs uep
      JOIN program_weeks pw ON pw.program_id = uep.program_id
      JOIN program_week_days pwd ON pwd.week_id = pw.id
     WHERE uep.profile_id = v_user_id
       AND uep.is_active = TRUE
       AND pwd.routine_id = v_routine_id
     LIMIT 1;
  END IF;

  -- ── 1. Insert workout_session ──────────────────────────────────────────────
  INSERT INTO workout_sessions (
    profile_id, gym_id, routine_id, program_enrollment_id, name, status,
    started_at, completed_at, duration_seconds, total_volume_lbs
  ) VALUES (
    v_user_id, v_gym_id, v_routine_id, v_enrollment_id, v_routine_name, 'completed',
  ...
  -- PR XP — capped at pr_max_per_session
  IF p_payload->'session_prs' IS NOT NULL AND json_array_length(p_payload->'session_prs') > 0 THEN
    v_pr_count := 0;
    FOR v_pr IN SELECT * FROM json_array_elements(p_payload->'session_prs')
    LOOP
